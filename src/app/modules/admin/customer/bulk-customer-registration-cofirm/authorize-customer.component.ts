...
export class AuthorizeCustomerComponent implements OnInit {
  ...
  displayedColumns: string[] = ["select_all",'Name','email','account_no','phone'];
  ...
  authorizeCustomer(userDetails : any) : void 
  {
      const dialogRef = this.confirmationDialog.open({
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
              ...
              const customer = {
                accountNumber : userDetails.cust_No,
   ...
   authorizeAccount(userDetails : any) : void 
  {
      const dialogRef = this.confirmationDialog.open({
        "title": "Authorize Customer",
        "message": `Are you sure you want to authorise this customer Account </br><span class="text-lg font-semibold">${userDetails.cust_ac_no} ?</span>`,
        "icon": {
          "show": true,
          "name": "mat_outline:library_add_check",
          "color": "info"
        ...
        dialogRef.afterClosed().subscribe((result) => {
            if (result === 'confirmed') {
              this.splashScreen.show();
              const customer = {
                accountNumber : userDetails.cust_ac_no,
                adminUsername : this.username,
                Name     : userDetails.ac_desc,
                id            : userDetails.id,
                // lastname      : userDetails.lname,
  ...
  customerIsClicked(acountDetails: any): void {
    if (acountDetails.selected) {
        this.selectedRecord.push(acountDetails);
